{{--
  ...
  Desktop: featured + uniform grid. Mobile: Swiper slider with light motion.
--}}
<section class="projects-section py-24 lg:py-32" id="projects">
  <div class="section-inner">
    ...
    <div class="filters-row" data-reveal id="projectFilters">
      <button type="button" class="filter-pill is-active" data-filter="all">{{ __('messages.filter_all') }}</button>
      @foreach(\App\Models\Project::CATEGORIES as $key => $names)
        <button type="button" class="filter-pill" data-filter="{{ $key }}">{{ $names[app()->getLocale()] }}</button>
      @endforeach
    </div>
    ...
    @php
      ...
      $arrowRtl = app()->getLocale() === 'ar';
    @endphp

    {{-- ── Desktop: featured + grid ─────────────────────────────────── --}}
    <div class="projects-desktop" id="projectsDesktop">
      @if($featured)
      ...
      <div class="projects-grid-clean" id="projectsGrid" data-stagger>
        @foreach($rest as $project)
          @include('sections.partials.project-tile', ['project' => $project, 'arrowRtl' => $arrowRtl])
        @endforeach
      </div>
    </div>

    {{-- ── Mobile: Swiper ───────────────────────────────────────────── --}}
    <div class="projects-mobile" id="projectsMobile">
      <div class="swiper projects-swiper" id="projectsSwiper" dir="{{ $arrowRtl ? 'rtl' : 'ltr' }}" data-lenis-prevent>
        <div class="swiper-wrapper">
          @foreach($projects as $project)
            <div class="swiper-slide" data-cat="{{ $project->category }}">
              @include('sections.partials.project-tile', ['project' => $project, 'arrowRtl' => $arrowRtl])
            </div>
          @endforeach
        </div>
        <div class="projects-swiper-footer">
          <div class="swiper-pagination projects-swiper-pagination"></div>
          <div class="projects-swiper-nav">
            <button type="button" class="projects-swiper-btn" id="projectsSwiperPrev" aria-label="{{ __('messages.prev') }}">
            ...
            <button type="button" class="projects-swiper-btn" id="projectsSwiperNext" aria-label="{{ __('messages.next') }}">
              <svg width="18" height="18" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                @if($arrowRtl)
                  <path d="M10 8H3M6 5l-3 3 3 3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                @else
    ...
    <div class="text-center mt-12" data-reveal>
      <a href="{{ route('projects.index', app()->getLocale()) }}" class="btn btn-ghost">
        {{ __('messages.projects_cta') }}
        <i data-lucide="arrow-{{ $arrowRtl ? 'left' : 'right' }}" class="w-4 h-4"></i>
      </a>
    </div>
    @endif
