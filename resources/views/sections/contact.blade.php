  ...
  <div class="section-inner">
    <div class="grid lg:grid-cols-2 gap-12 items-center">
      <div data-reveal="left">
        <span class="eyebrow" style="background:rgba(229,57,53,.15);color:var(--mi-red-light)">{{ __('messages.contact_eyebrow') }}</span>
        <h2 class="display-2 mt-2" style="color:#fff" data-reveal="title">{!! __('messages.contact_title') !!}</h2>
        <p class="lead mt-5" style="color:rgba(255,255,255,.72)" data-reveal data-reveal-delay="0.1">{{ __('messages.contact_blurb') }}</p>

        <div class="mt-10 space-y-5" data-reveal data-reveal-delay="0.2">
          <a href="tel:{{ config('mi.phone_primary') }}" class="flex items-center gap-4 group">
            ...
            <div>
              <div class="label-mono" style="color:rgba(255,255,255,.5)">{{ __('messages.call_us') }}</div>
              <div style="font-weight:700;font-size:20px" dir="ltr">{{ config('mi.phone_primary') }}</div>
            </div>
          </a>
        </div>
      </div>
      ...
      <div data-reveal="right">
        <form method="POST" action="{{ route('contact.store', $locale) }}"
          ...
          <div class="grid sm:grid-cols-2 gap-5 mb-5">
            <label class="block">
              <span class="label-mono" style="color:rgba(255,255,255,.55)">{{ __('messages.field_name') }}</span>
            ...
            <label class="block">
              <span class="label-mono" style="color:rgba(255,255,255,.55)">{{ __('messages.field_company') }}</span>
            ...
            <label class="block">
              <span class="label-mono" style="color:rgba(255,255,255,.55)">{{ __('messages.field_email') }}</span>
            ...
            <label class="block">
              <span class="label-mono" style="color:rgba(255,255,255,.55)">{{ __('messages.field_phone') }}</span>
          ...
          <label class="block mb-5">
            <span class="label-mono" style="color:rgba(255,255,255,.55)">{{ __('messages.field_flock') }}</span>
          ...
          <label class="block mb-5">
            <span class="label-mono" style="color:rgba(255,255,255,.55)">{{ __('messages.field_message') }}</span>
          ...
          <button type="submit" class="btn btn-primary w-full" style="padding:16px">
            {{ __('messages.send_request') }} <i data-lucide="send" class="w-4 h-4"></i>
          </button>
          @if(session('contact_ok'))
            <p class="text-sm mt-4" style="color:var(--mi-red-light)">✔ {{ session('contact_ok') }}</p>
          @endif
        </form>
      </div>
    </div>
