      ...
      <div class="relative" data-reveal="left">
        ...
        <div data-reveal="scale" data-reveal-delay="0.25" style="position:absolute;top:-20px;right:-20px;width:120px;height:120px;">
          ...
          <div>
            ...
            <div class="label-mono" style="color:rgba(255,255,255,.6);margin-top:4px;font-size:9px">{{ __('messages.about_badge') }}</div>
          </div>
          </div>
        </div>
      </div>
      <div data-reveal="right">
        <span class="eyebrow">{{ __('messages.about_eyebrow') }}</span>
        <h2 class="display-2 mt-2" data-reveal="title">{!! __('messages.about_title') !!}</h2>
        <p class="lead mt-6" data-reveal data-reveal-delay="0.1">{{ __('messages.about_blurb') }}</p>
        <a href="{{ route('about', app()->getLocale()) }}" class="btn btn-primary mt-8" data-magnetic data-reveal data-reveal-delay="0.2">
          {{ __('messages.about_cta') }} <i data-lucide="arrow-left" class="w-4 h-4"></i>
        </a>
      </div>
    </div>
  </div>
